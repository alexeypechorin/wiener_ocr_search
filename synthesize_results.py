...
def aggregate_metrics(parent_dir, metrics):
    ...
    metrics_files = os.listdir(parent_dir)
    metrics_files = [metric_file for metric_file in metrics_files if metric_file.startswith('stats')]

    for metric_file in sorted(metrics_files):
        path_to_metric_file = os.path.join(parent_dir, metric_file)
        if os.path.isfile(path_to_metric_file):
            with open(path_to_metric_file, 'r') as f:
                metric_file_name = metric_file.split('.')[0].split('_')[1]
                metrics[parent_dir + '-' + metric_file_name] = json.load(f)

    # Check every subdirectory of parent_dir
    for subdir in list(sorted(os.listdir(parent_dir))):
...
def metrics_to_table(metrics):
    ...
    headers = list(sorted(metrics[list(metrics.keys())[0]].keys()))
    table = [[subdir] + [values[h] for h in headers] for subdir, values in sorted(list(metrics.items()))]
